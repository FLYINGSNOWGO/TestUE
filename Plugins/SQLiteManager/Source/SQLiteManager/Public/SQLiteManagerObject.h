...
#include "SQLiteManagerObject.generated.h"

class FSQLiteDatabase;
USTRUCT(BlueprintType)
struct SQLITEMANAGER_API FSQLiteDatabaseReference
{
...
USTRUCT(BlueprintType)
struct SQLITEMANAGER_API FSQLiteKeyValuePair
{
	GENERATED_USTRUCT_BODY()
...
USTRUCT(BlueprintType)
struct SQLITEMANAGER_API FSQLiteQueryResultRow
{
	GENERATED_USTRUCT_BODY()
...
USTRUCT(BlueprintType)
struct SQLITEMANAGER_API FSQLiteQueryResult
{
	GENERATED_USTRUCT_BODY()
 ...
 */
UCLASS()
class SQLITEMANAGER_API USQLiteManagerObject : public UObject
{
	GENERATED_BODY()

public:
	UFUNCTION(BlueprintCallable, BlueprintPure, Category = "SQLite")
		static USQLiteManagerObject* GetSQLiteManagerInstance();

	/** Add a database to the list of databases. It will be checked that it's valid (will try to open it and keep open) */
	...
	*   Note: Does not create a new object. ObjectToPopulate is the reference to the object you want to populate. */
	...
	static bool InsertRowsIntoTable(const FString DatabaseName, const FString TableName, const TArray<FSQLiteTableRowSimulator> RowsOfFields);
...
private:
	//FSQLiteDatabase SQLiteCoreDatabase;
	/** A list of the databases for convenience, easier to refer to them by name rather than a long filename. */
	static TMap<FString, TSharedRef<FSQLiteDatabase>> Databases;
	static USQLiteManagerObject* Singleton;
private:
	USQLiteManagerObject(const FObjectInitializer& ObjectInitializer);

	virtual ~USQLiteManagerObject();
	/*
		Checks database validity (if the file exists and/or if it can be opened).
		If TestByOpening = true and the file exists, the connection state is maintained
	*/
	static bool IsValidDatabase(const FString* Name, const FString* Filename, bool RelativeToGameContentDirectory, bool TestByOpening);

	/** Tries to open or create a database. */
	static bool OpenDatabase(const FString* Name, const FString* ActualFilename);

	static void CreateDatabase(const FString* Name, const FString* Filename, bool RelativeToGameContentDirectory);
	...
	static bool PrepareStatement(const FString* DatabaseName, const FString* Query, FSQLitePreparedStatement &SQLitePreparedStatement);
};
