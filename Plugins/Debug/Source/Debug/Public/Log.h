...
/**
 * scope log
 * When entering the scope, an Enter identifies the log
 * Exit indicates the log when exiting the scope
 */
class DEBUG_API FScopeLog
{
public:
	FScopeLog(FString LogStr, ELoggingOptions Options = ELoggingOptions::LO_Console,bool IsAllowCDOLog= true,UObject* Object = nullptr);
	~FScopeLog();
private:
	FString Log;
	ELoggingOptions LoggingOptions;
	bool bShowLog = true;
};
