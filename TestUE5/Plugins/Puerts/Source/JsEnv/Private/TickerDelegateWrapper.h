...
public:
    ...
    void Init(const v8::FunctionCallbackInfo<v8::Value>& Info, std::function<void(v8::Isolate*, v8::TryCatch*)> InExceptionHandler,
        std::function<void(FTSTicker::FDelegateHandle*)> InDelegateHandleCleaner);

    // 调用JS函数
    bool CallFunction(float);

    void SetDelegateHandle(FTSTicker::FDelegateHandle* Handle);
...
private:
    ...
    std::function<void(FTSTicker::FDelegateHandle*)> DelegateHandleCleaner;
