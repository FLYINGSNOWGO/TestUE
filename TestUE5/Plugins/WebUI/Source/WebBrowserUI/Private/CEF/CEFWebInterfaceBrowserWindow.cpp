...
bool FCEFWebInterfaceBrowserWindow::CreateInitialTextures()
{
	if (FSlateApplication::IsInitialized())
	{
		if (FSlateRenderer* Renderer = FSlateApplication::Get().GetRenderer())
		{
			...
			{
				if (FCEFWebInterfaceBrowserWindowRHIHelper::BUseRHIRenderer() && RHIRenderHelper == nullptr)
				{
					RHIRenderHelper = new FCEFWebInterfaceBrowserWindowRHIHelper;
				}
...
TSharedRef<SViewport> FCEFWebInterfaceBrowserWindow::CreateWidget()
{
	TSharedRef<SViewport> BrowserWidgetRef =
		...
		.RenderTransform(this, &FCEFWebInterfaceBrowserWindow::GetWebBrowserRenderTransform);
...
TOptional<FSlateRenderTransform> FCEFWebInterfaceBrowserWindow::GetWebBrowserRenderTransform() const
{
...
#define LETTER_KEY_MACRO(val, vkey) else if(Key == EKeys::val) \
	...
	//UE_LOG(LogWebInterfaceBrowser, Log, TEXT("Modifiers: %i %i %i") , OutKeyEvent.unmodified_character, OutKeyEvent.windows_key_code, OutKeyEvent.modifiers);
...
void FCEFWebInterfaceBrowserWindow::GetViewRect(CefRect& Rect)
{
...
void FCEFWebInterfaceBrowserWindow::OnPaint(CefRenderHandler::PaintElementType Type, const CefRenderHandler::RectList& DirtyRects, const void* Buffer, int Width, int Height)
{
	...
	if (bUsingAcceleratedPaint)
	{
		UE_LOG(LogWebInterfaceBrowser, Error, TEXT("Accelerated CEF rendering selected but OnPaint called. Disabling accelerated rendering for this browser window."));
...
void FCEFWebInterfaceBrowserWindow::OnAcceleratedPaint(CefRenderHandler::PaintElementType Type, const CefRenderHandler::RectList& DirtyRects, void* SharedHandle)
{
	bool bNeedsRedraw = false;
	if (!bUsingAcceleratedPaint)
	{
		UE_LOG(LogWebInterfaceBrowser, Error, TEXT("Accelerated CEF rendering NOT selected but OnAcceleratedPaint called. Enabling accelerated rendering for this browser window."));
...
bool FCEFWebInterfaceBrowserWindow::OnBeforeBrowse( CefRefPtr<CefBrowser> Browser, CefRefPtr<CefFrame> Frame, CefRefPtr<CefRequest> Request, bool user_gesture, bool bIsRedirect )
{
...
FString _URLRequestStatusToString(const CefResourceRequestHandler::URLRequestStatus& Status)
{
...
void FCEFWebInterfaceBrowserWindow::HandleOnResourceLoadComplete(const CefString& URL, CefRequest::ResourceType Type, CefResourceRequestHandler::URLRequestStatus Status, int64 ContentLength)
{
...
EWebInterfaceBrowserConsoleLogSeverity CefLogSeverityToWebBrowsers(cef_log_severity_t Level)
{
	switch (Level)
	{
	case LOGSEVERITY_VERBOSE:
		return EWebInterfaceBrowserConsoleLogSeverity::Verbose;
		//case LOGSEVERITY_DEBUG: // same as LOGSEVERITY_VERBOSE
		//	return Verbose;
	case LOGSEVERITY_INFO:
		return EWebInterfaceBrowserConsoleLogSeverity::Info;
	case LOGSEVERITY_WARNING:
		return EWebInterfaceBrowserConsoleLogSeverity::Warning;
	case LOGSEVERITY_ERROR:
		return EWebInterfaceBrowserConsoleLogSeverity::Error;
	case LOGSEVERITY_FATAL:
		return EWebInterfaceBrowserConsoleLogSeverity::Fatal;
	case LOGSEVERITY_DEFAULT:
	default:
		return EWebInterfaceBrowserConsoleLogSeverity::Default;
	}
}

void FCEFWebInterfaceBrowserWindow::HandleOnConsoleMessage(CefRefPtr<CefBrowser> Browser, cef_log_severity_t Level, const CefString& Message, const CefString& Source, int Line)
{
	ConsoleMessageDelegate.ExecuteIfBound(WCHAR_TO_TCHAR(Message.ToWString().c_str()), WCHAR_TO_TCHAR(Source.ToWString().c_str()), Line, CefLogSeverityToWebBrowsers(Level));
}
...
bool FCEFWebInterfaceBrowserWindow::CanSupportAcceleratedPaint()
{
...
bool FCEFWebInterfaceBrowserWindow::OnProcessMessageReceived(CefRefPtr<CefBrowser> Browser, CefRefPtr<CefFrame> frame, CefProcessId SourceProcess, CefRefPtr<CefProcessMessage> Message)
{
